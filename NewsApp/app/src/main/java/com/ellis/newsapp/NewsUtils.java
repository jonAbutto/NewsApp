...
public class NewsUtils {
    ...
    //perform a query  to the Guardian Api and returna list of {@NewsCustomClass} objects
...
//   create url object
    ...
    private static String makeHttpRequest(URL url) throws IOException {
        ...
        try {
            ...
            if (urlConnection.getResponseCode() == RESPONSE_CODE_OK) {
                ...
                Log.e(LOG_TAG, "Error Repsonse Code: " + urlConnection.getResponseCode());
            }
        } catch (IOException e) {
     ...
     */
    public static List<NewsCustomClass> extractFeaturedFromJson(String newsJson) {
        ...
        //try catch block to handle exceotions that may
        ...
        try {
            ...
            JSONObject newsFeeds = baseObject.optJSONObject("response");
            //get the jsonarray items
            JSONArray newsArray = newsFeeds.optJSONArray("results");
            //looping through te news items

            for (int i = 0; i < newsArray.length(); i++) {
                // getting the JSONObject and all its attributes
                JSONObject allNewsFeeds = newsArray.optJSONObject(i);
                //find the key for the author
                JSONObject authorKey = allNewsFeeds.optJSONObject("fields");
